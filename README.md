...
- 展览管理：创建、编辑、删除展览，调整展品顺序，为每件展品设置入展 / 撤展日期、主题色并发布。
- 档期校验：草稿阶段允许预留冲突；发布时校验重复选品、跨展览档期碰撞与日期完整性，失败则保留原草稿和线上展览并逐条说明冲突。
- 按日开放：展厅只进入当天开放的已发布展览，未开放展览显示开放日期；撤展后的作品不再出现在路线中，没有档期的旧展览按常设展开放。
